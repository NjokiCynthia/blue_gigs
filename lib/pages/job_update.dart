  ...
  Future<void> updatePost() async {
    ...
    for (var item in list) {
      if (item["postId"] == widget.upload_id && item["userId"] == uid) {
        item["job"] = !job2.isEmpty ? job2 : widget.job;
        item["currency"] = !currency2.isEmpty ? currency2 : widget.currency;
        item["duration"] = !duration2.isEmpty ? duration2 : widget.duration;
        item["amount"] = amountCtrl.text.length == 0
            ? widget.amount
            : amountCtrl.text.toString();

        await reference.doc(item["postId"]).update(item);
      }
    }
  }
